...
function sketch(num, sz, color, h, w) {
  ...
  function Circle(radius, speed, width, xPos, yPos) {
    ...
    this.xPos = xPos;
    this.yPos = yPos;
    this.opacity = .05 + Math.random() * .8;                    //opacity
  ...
  Circle.prototype.update = function() {
    ...
    mainContext.arc(
      this.xPos + Math.cos(this.counter / w) * this.radius,
  ...
  var szNum;
  if (sz == "fkbig") { szNum = 25; }
    else if (sz == "big") { szNum = 10; }
  ...
  function drawCircles() {
    for (var i = 0; i < num; i++) {
      ...
      var speed = 0.3 + Math.random() * 3; //min 0.3 max 0.3+2=2.3                    ///control speed
