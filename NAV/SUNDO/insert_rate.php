...
if (($_SERVER['REQUEST_METHOD'] == 'POST')) {
    if (isset($_POST['remove_rate'])) {
        function remove_rate($feedback_id){
            $dbname = 'webappdb';
            $tablename = 'feedback';
            ...
            if (!mysqli_connect($servername, $username, $password)){
                ...
                $sql = "DELETE FROM $tablename WHERE feedback_id = '$feedback_id';";
                mysqli_query($con, $sql);
                echo "
                <script> alert('review and rating is deleted') </script>
                ";
        }

        $feedback_id = $_POST['feedback_id'];
        remove_rate($feedback_id);
